...
class project():
	def __init__(self, name, rconfig, pconfig):
		# Store each of the properties of the project for later use
		self.name = name
		self.fetch = pconfig.get("fetch", rconfig["fetch"])
		# hen detect the valid fetching method even when fetching is disabled because is needed on presentation
		self.fetcher = self.getfetchmethod()
		self.preserve = pconfig.get("preserve", rconfig["preserve"])
		...
		self.build = pconfig.get("build", rconfig["build"])
		self.tasks = pconfig.get("tasks", rconfig["tasks"])
		self.keystore = pconfig.get("keystore", rconfig["keystore"])
		self.keyalias = pconfig.get("keyalias", rconfig["keyalias"])
		self.deploy = pconfig.get("deploy", rconfig["deploy"])
		# Some properties are only generated by syncall in the scope of retrying so avoid user interference
	...
	def package(self, command):
		...
		if os.path.isfile("./entrypoint-syncall"):
			...
			setup = subprocess.call(["./entrypoint-syncall"], stdout = self.logfile, stderr = subprocess.STDOUT)
			if setup != 0:
				print("- \033[91mFAILED\033[0m")
	...
	def sign(self, workdir, signinfo, alias):
		releases = []
		...
		for apk in apks:
			...
			if verify == 1:
				...
				sign = subprocess.Popen(["apksigner", "sign", "--ks", signinfo["path"], "--ks-key-alias", alias,"--out", workdir + "/SYNCALL-RELEASES/" + displayname, "--in", apk], stdout = self.logfile, stdin=subprocess.PIPE, stderr=subprocess.STDOUT)
		...
		for apk in self.deploylist:
			...
			for target in self.deploylist[apk]:
				...
				# If the adb subprocess timed out we skip this device
					...
					send = subprocess.call(["adb", "-s" , target, "install", "-r", workdir + "/SYNCALL-RELEASES/" + apk], stdout = self.logfile, stderr=subprocess.STDOUT)
					if send == 0:
