...
class project():
	...
	def fetch(self):
		...
		if self.fetcher.fetch(self.logfile):
			...
			checker = self.fetcher.newtag(self.tagregex) if self.followtags else self.fetcher.updated()
			if checker[0]:
				...
				if merger:
					...
					# Merging went bad
					self.failed.update({"sync": self.sync, "followtags": self.followtags, "preserve": self.preserve, "tagregex": self.tagregex, "subdir": self.subdir, "javahome": self.javahome, "build": self.build, "force": self.force, "tasks": self.tasks, "keystore": self.keystore, "keyalias": self.keyalias, "signlist": self.signlist, "deploylist": self.deploylist, "deploy": self.deploy})
					cprint("FAILED", "error")
				...
				if self.preserve and diff.decode() != "":
					...
					if self.fetcher.restore(diff, self.logfile):
						...
						self.failed.update({"sync": not self.changed, "followtags": self.followtags, "preserve": self.preserve, "tagregex": self.tagregex, "subdir": self.subdir, "javahome": self.javahome, "build": self.build, "force": self.changed, "tasks": self.tasks, "keystore": self.keystore, "keyalias": self.keyalias, "signlist": self.signlist, "deploylist": self.deploylist, "deploy": self.deploy})
						self.build = False
						cprint("FAILED", "error")
			else:
