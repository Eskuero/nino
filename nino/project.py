			...
			if self.fetcher.updated():
				...
				if self.preserve and diff.decode() != "":
					...
					if self.fetcher.restore(diff, self.logfile):
						...
						self.failed.update({"sync": not self.changed, "preserve": self.preserve, "build": self.build, "force": self.changed, "tasks": self.tasks, "keystore": self.keystore, "keyalias": self.keyalias, "signlist": self.signlist, "deploylist": self.deploylist, "deploy": self.deploy})
						self.build = False
						cprint("FAILED", "error")
			else:
